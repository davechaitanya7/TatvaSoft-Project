...
namespace Data_Access_Layer
{
    public class DALLogin
    {
        ...
        public async Task<string> UpdateUserAsync(User user)
        {
            ...
            try
            {
                var userUp = _cIDbContext.User.Where(x => x.Id == user.Id).FirstOrDefault();

                if (userUp != null)
                {
                    userUp.PhoneNumber = user.PhoneNumber;
                    userUp.EmailAddress = user.EmailAddress;
                    userUp.ConfirmPassword = user.ConfirmPassword;
                    userUp.EmailAddress = user.EmailAddress;
                    userUp.FirstName = user.FirstName;
                    userUp.LastName = user.LastName;
                    userUp.Password = user.Password;
                    _cIDbContext.SaveChanges();
                    result = "Update Deatils Sucesfully ";
                    return result;
                }
                else
                {
            ...
            {
                throw new Exception("Error in Do Processs");
            }
        }
