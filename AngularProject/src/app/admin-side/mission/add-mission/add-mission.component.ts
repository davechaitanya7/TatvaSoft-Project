...
export class AddMissionComponent implements OnInit {
  ...
  addMissionFormValid() {
    this.addMissionForm = this.fb.group({
      ...
      missionOrganizationName: [null, Validators.compose([Validators.required])],
      missionOrganizationDetail: [null, Validators.compose([Validators.required])],
      missionType: [null, Validators.compose([Validators.required])],
      missionDocumnets: [null, Validators.compose([Validators.required])],
  ...
  get missionOrganizationName() { return this.addMissionForm.get('missionOrganizationName') as FormControl}
  get missionOrganizationDetail() { return this.addMissionForm.get('missionOrganizationDetail') as FormControl}
  get missionType() { return this.addMissionForm.get('missionType') as FormControl; }
  get missionDocumnets() { return this.addMissionForm.get('missionDocumnets') as FormControl;}
  ...
  OnSelectedDocuments(event: any) {
    const files = event.target.files;
    if (this.DocumentListArray.length > 1) {
      return this.toast.error({ detail: "ERROR", summary: "Maximum 1 Files can be added.", duration: 3000 });
    }
    if (files) {
      this.formData = new FormData();
      for (const file of files) {
  ...
  async OnSubmit() {
    ...
    if (this.addMissionForm.valid) {
      ...
      if (this.DocumentListArray.length > 0) {
        await this.service.UploadDoc(this.formData).pipe().toPromise().then((res: any) => {
          if (res.result == 1) {
      ...
      value.missionDocumnets =DocumentUrl;
